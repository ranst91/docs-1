...
description: Integrate with the Writer PDF parser parsers document loader using LangChain
  Python.
integration:
  name: PDFParser
...
It's also helpful (but not needed) to set up [LangSmith](https://smith.langchain.com?utm_source=docs&utm_medium=cta&utm_campaign=langsmith-signup&utm_content=oss-python-integrations-document_loaders-parsers-writer_pdf_parser) for best-in-class observability. If you wish to do so, you can set the `LANGSMITH_TRACING` and `LANGSMITH_API_KEY` environment variables:
